...
def register_user(user_conn, client_msg_payload):
    ...
    if new_username != "" and new_username not in clients_names:
...
def send_file_crc(user_conn, client_msg_payload, user_uuid, aes_key):
    ...
    cipher_decrypt = AES.new(aes_key, AES.MODE_CBC, iv=iv)
    decrypt_file_data = unpad(cipher_decrypt.decrypt(file_data), AES.block_size)
    ...
    send_msg_to_client(user_conn, struct.pack('<BHI16sI255sI', 3, MsgTypes.SEND_CRC.value,
                                              ...
                                              file_data_crc))
    return decrypt_file_data.decode()
...
def save_file(client_msg_payload, user_uuid_str, file_data):
...
def user_communication(user_conn):
    ...
    while True:
        try:
            client_response = get_client_msg(user_conn)
            if client_response == -1:
                print("Error: faild to get client msg, the client may have disconnected")
                break
            client_msg_header, client_msg_payload = client_response
            user_uuid = client_msg_header["ClientId"]
            ...
            if request_code == RequestType.REGISTER.value:
                ...
                if not user_uuid:
                    ...
                    send_msg_to_client(user_conn, b"FAILD")
                else:
                ...
                if not decrypt_data:
                    ...
                    is_save_file_succeeded = save_file(client_msg_payload, str(user_uuid), decrypt_data)
